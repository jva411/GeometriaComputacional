...
use crate::scene::{scene::Scene, ui::{light_ui::NewLightProperties, object_ui::NewObjectProperties}, window::Window};
...
impl SelectedObject {
  pub fn try_get_id(&self) -> Option<Uuid> {
    match self {
      SelectedObject::Object(id) => Some(*id),
      SelectedObject::Light(id) => Some(*id),
      ...
      .show(&self.egui.context, |ui| {
        if let Some(id) = ui_manager.selected_object.try_get_id() {
          if let Some(transformable) = scene.get_transformable_by_id(id) {
            ...
            let view_matrix: [[f64; 4]; 4] = scene.camera.get_view().transpose().as_dmat4().to_cols_array_2d();
            let proj_matrix: [[f64; 4]; 4] = scene.camera.get_projection().transpose().as_dmat4().to_cols_array_2d();

            let gizmo_mode = match ui_manager.selected_object {
              SelectedObject::Object(_) => GizmoMode::all(),
              SelectedObject::Light(_) => GizmoMode::all_translate(),
              _ => GizmoMode::all(),
            };
            ui_manager.gizmo.update_config(GizmoConfig {
            ...
            let gizmo_transform = transform_gizmo_egui::math::Transform::from_scale_rotation_translation(
              ...
              transform.rotation.to_quat().as_dquat().to_array(),
              transform.translation.as_dvec3().to_array(),
            );

            if let Some((_result, new_transforms)) = ui_manager.gizmo.interact(ui, &[gizmo_transform]) {
              let new_transform = new_transforms[0];
              let (scale, rotation, translation) = (new_transform.scale, new_transform.rotation, new_transform.translation);

              let tx: [f64; 3] = translation.into();
              transform.translation = glam::Vec3::new(tx[0] as f32, tx[1] as f32, tx[2] as f32);

              let sx: [f64; 3] = scale.into();
              transform.scale = glam::Vec3::new(sx[0] as f32, sx[1] as f32, sx[2] as f32);

              let rx: [f64; 4] = rotation.into();
              let rot_quat = glam::Quat::from_array([rx[0] as f32, rx[1] as f32, rx[2] as f32, rx[3] as f32]);

              let (yaw, pitch, roll) = rot_quat.to_euler(glam::EulerRot::YXZ);
              transform.rotation.yaw = yaw.to_degrees();
              transform.rotation.pitch = pitch.to_degrees();
              transform.rotation.roll = roll.to_degrees();
            }

            ui_manager.gizmo.draw();
          }
        }
      });
      ...
      match command {
        ...
        UICommand::CreateConvexHull(selected_object) => {
          match selected_object {
            ...
            SelectedObject::Object(id) => { self.create_convex_hull(id); }
            _ => {}
          }
        }
